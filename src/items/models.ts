...
import { box, ball, material, mesh, woodenShield, woodenSword } from "../game/models";
...
export function itemModel(id: ItemId) {
  ...
  if (id === "royal-crown") {
    ...
    const rim = mesh(new THREE.TorusGeometry(0.34, 0.033, 6, 16), material("#71323c"), root, 0, -0.065, 0);
    rim.rotation.x = Math.PI / 2;
    const ruby = material("#c32040", 0.35);
    const band = mesh(new THREE.CylinderGeometry(0.34, 0.34, 0.16, 12, 1, true), gold, root, 0, 0.01, 0);
    gold.side = THREE.DoubleSide;
    band.name = "crown-band";
    for (let i = 0; i < 6; i++) {
      const angle = i * Math.PI / 3;
      const point = mesh(new THREE.ConeGeometry(0.1, 0.25, 4), gold, root,
        Math.sin(angle) * 0.31, 0.2, Math.cos(angle) * 0.31);
      point.rotation.y = angle;
      const gem = mesh(new THREE.OctahedronGeometry(0.073), ruby, root,
        Math.sin(angle) * 0.355, 0.055, Math.cos(angle) * 0.355);
      gem.scale.set(1, 1.25, 0.65);
      gem.rotation.y = angle;
      gem.name = "crown-ruby";
      ball(root, gold, Math.sin(angle) * 0.31, 0.34, Math.cos(angle) * 0.31, 0.035);
    }

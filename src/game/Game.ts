import { RijuCompanion } from "./companions/RijuCompanion";
...
import { SidonCompanion } from "./companions/SidonCompanion";
export class Game {
  ...
  private riju = new RijuCompanion();
  private sidon = new SidonCompanion();
  ...
  constructor(
    ...
    this.scene.add(this.sidon.root, this.riju.root);
    this.mountArea();
  ...
  private mountArea() {
      ...
      !gameStore.getState().location && this.previousLocation?.castle === "hall"
        ...
        : gameStore.getState().location?.castle === "hall" &&
          ...
          : gameStore.getState().location?.castle === "hall" && this.previousLocation?.castle === "throne"
            ? { x: 0, z: -2.5 } : null;
    const spawn =
      natureArrival(gameStore.getState().location, this.previousLocation) ??
    ...
    this.sidon.reset(this.player.position, this.world);
    this.riju.reset(this.player.position, this.world);

...
export class VolcanoInteriorArea implements Area {
  ...
  constructor() {
    ...
    const stone = material("#554951"), floor = material("#75665e");
    ...
    [[-6.4, 5.7], [-6.9, 3.4], [-6.5, 0], [-6.8, -3.2], [-6.2, -9.3],
      ...
      [6.7, 1.6], [6.5, 5.8], [3.4, 6.55], [-1.4, 6.45]].forEach(([x, z], i) =>
    ...
    for (const side of [-1, 1]) {
      ...
      rockWall(this.root, this.collision, strata[side === 1 ? 1 : 2],
        side * 2.95, -4.8, 0.9, 1.2, side === 1 ? 3.8 : 3.5, side + 5);
    }
    // Overlapping, uneven rock shoulders replace the straight lintel and posts.
    const archGeometry = new THREE.DodecahedronGeometry(1, 0);
    for (let i = 0; i < 5; i++) {
      const cap = mesh(archGeometry, strata[i % 3], this.root,
        -2.4 + i * 1.2, 3.65 + Math.sin(i * 1.8) * 0.22, -5.05 + Math.cos(i) * 0.12);
      cap.scale.set(1.05, 0.68 + i % 2 * 0.18, 0.95);
      cap.rotation.set(i * 0.12, i * 0.7, 0.15 * Math.sin(i * 2));
    }
    ...
    caveMouth(this.root, this.collision, VOLCANO_EXIT, true);
    ...
    path([[-0.8, 6.4], [-1.0, 3], [-0.86, 0.8], [-1.04, -2.6], [-0.9, -8.7],
      [0.8, -8.7], [0.92, -3], [0.86, 0], [1.02, 2.5], [0.78, 6.4]]);
  ...
  passages(): Passage[] {
    const fire = DUNGEONS.find((d) => d.id === "fire")!;
    return [
      ...(!this.yunoboRock.blocking ? [{ x: 0, z: -8, destination: { world: "water" as const } }] : []),
      { ...VOLCANO_EXIT, destination: { world: "volcano" } },
  ...
  update(dt: number, time: number) {
    ...
    this.collision.dynamic = this.yunoboRock.blocking ? [{ ...YUNOBO_ROCK, halfX: 2.5, halfZ: 0.65 }] : [];
    this.glow.emissiveIntensity = 0.7 + Math.sin(time * 1.4) * 0.15;
  }
  dispose() { this.yunoboRock.dispose(); disposeTree(this.root); }
}

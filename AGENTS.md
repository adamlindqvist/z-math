...
- For either verification method, cover only affected interactions. Check simultaneous touches and canceled input when controls change; dialogs and pausing when those flows change. Play through the chest reward only when progression or rewards change, and reload when saved state changes.
